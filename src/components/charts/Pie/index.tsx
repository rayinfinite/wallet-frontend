...
import { FieldString, TooltipCallback } from 'bizcharts/lib/interface';
...
const Pie: React.FC<ChartProps> = (props) => {
  const { data, loading = false } = props;
  const [legendData, setLegendData] = useState<DataItem[]>([]);
  const [chart, setChart] = useState<typeof Chart | undefined>();
  ...
  const getG2Instance = (chart: typeof Chart) => {
    setChart(chart);
  };
  useEffect(() => {
    getLegendData();
  ...
  const tooltipFormat: [FieldString, TooltipCallback] = [
    'x*y',
    (x: string, y: number) => ({
      name: x,
      value: y,
  ...
  useEffect(() => {
    setTotal(parseFloat(data.reduce((pre, now) => now.y + pre, 0).toFixed(2)));
  }, [data]);

  const legendClickHandler = (item: DataItem, i: number) => {
    const newItem = item;
    newItem.checked = !newItem.checked;
    const newLegendData: DataItem[] = [...legendData];
